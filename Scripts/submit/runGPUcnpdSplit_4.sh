...
do
	...
	# Copy input file.
	cat inputO3.in > mode$(($i+1))/inputMeph_$(($i+1)).in

	# Runs the calculation.
	cd mode$(($i+1))/
	./submitGPUcnpd.sh /home/gpu/proj/proj471/pbrandi/local/lib/openmpi-1.6.5/bin/mpirun 7 /home/gpu/proj/proj471/pbrandi/local/bin/i-disorder.optim . 1 inputMeph_$(($i+1)).in > IDisorder.log
